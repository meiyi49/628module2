...
# 提取特征和标签
train_features <- clean_data[, -c(1, 2)]  # 训练集特征，去掉ID列和BODYFAT列
train_labels <- clean_data$BODYFAT  # 训练集的BODYFAT值作为标签

test_features <- out_data[, -c(1, 2)]  # 测试集特征，去掉ID列和BODYFAT列
...
```{r}
# 合并 clean_data 和 out_data，并添加来源列
...
ggplot(combined_data, aes(x = BODYFAT, y = Predicted_BODYFAT, color = Source)) +
  geom_point(size = 3) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +  # 理想情况下的对角线
